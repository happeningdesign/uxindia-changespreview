...
function searchDay(
  ...
  for (const session of sessions) {
    ...
    if (
      ...
      speakerMatchesName(session.speaker.name, speakerName)
    ) {
    ...
    if (session.sessions) {
      for (const s of session.sessions) {
        if (
          s.speaker?.name &&
          speakerMatchesName(s.speaker.name, speakerName)
        ) {
          return {
            date,
            time,
      ...
      for (const p of session.rightSession.panelists) {
        if (p.name && speakerMatchesName(p.name, speakerName)) {
          return {
            ...
            time,
            endTime,
            title: session.rightSession.title,
            type: session.rightSession.tag ?? session.tag,
          };
        }
      }
    }
    ...
    if (session.workshops) {
      for (const w of session.workshops) {
        if (
          w.speaker?.name &&
          speakerMatchesName(w.speaker.name, speakerName)
        ) {
          return {
            date,
            time,
 ...
 */
export function lookupLeadershipSchedule(
  speakerName: string,
): ScheduleMatch | null {
 ...
 */
export function lookupRisingSchedule(
  speakerName: string,
): ScheduleMatch | null {
