...
import {
  risingLeadersSpeakersList,
  getRisingLeadersSpeakerBySlug,
} from "@/data/events/all-speakers";
...
export async function generateMetadata({
  ...
  const description =
    speaker.bio ||
    `${speaker.name}, ${speaker.role}${speaker.company ? ` at ${speaker.company}` : ""} — Speaker at UXINDIA 2026 Rising Leaders Forum.`;
...
function toTalkArray(
  t: SpeakerTalk | SpeakerTalk[] | undefined,
): SpeakerTalk[] {
  if (!t) return [];
  ...
  return (
    <div className={cardCls}>
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {talk.type &&
          (Array.isArray(talk.type) ? (
            talk.type.map((t, i) => (
              <TypeChip key={i} label={t} light={light} />
            ))
          ) : (
  ...
  return (
    <div className={`${bg} min-h-screen`}>
      ...
      <main className="max-w-5xl mx-auto px-6 md:px-10 pt-28 md:pt-36 pb-24">
        {/* Back */}
        <Link href="/rising-leaders-forum#speakers" className={backCls}>
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-16 items-start mb-12">
          ...
          <div className="flex flex-col justify-center gap-4 md:py-4 flex-1">
            ...
            {(hasLeadershipTalk || hasRisingTalk) &&
              (() => {
                ...
                const allTypes = Array.from(
                  new Set(
                    allEntries.flatMap((t) =>
                      Array.isArray(t.type) ? t.type : t.type ? [t.type] : [],
                    ),
                  ),
                );
                const heroTalk =
                  allEntries.find((t) => t.type !== "Workshop") ??
                  allEntries[0];
                if (!heroTalk && allTypes.length === 0) return null;
                return (
                  <div className="flex flex-wrap gap-2 items-center">
                    {hasLeadershipTalk && (
                      <EventChip event="leadership" light={isLight} />
                    )}
                    {hasRisingTalk && (
                      <EventChip event="rising" light={isLight} />
                    )}
                    {allTypes.map((v, i) => (
                      <TypeChip key={i} label={v} light={isLight} />
                    ))}
                    {heroTalk?.track && (
                      <TrackChip label={heroTalk.track} light={isLight} />
                    )}
                    {heroTalk?.talkCategory && (
                      <CategoryChip
                        label={heroTalk.talkCategory}
                        light={isLight}
                      />
                    )}
                  </div>
            ...
            {(() => {
              const heroTalk =
                [...leadershipTalks, ...risingTalks].find(
                  (t) => t.type !== "Workshop",
                ) ??
                leadershipTalks[0] ??
                risingTalks[0];
              return heroTalk?.title ? (
                <p
                  className={`flex items-center gap-1.5 font-sans text-base md:text-lg font-semibold text-balance break-words ${roleCls}`}
                >
                  <svg
                    width="13"
                    height="13"
        ...
        {hasTalks &&
          (() => {
            const allEntries = [...leadershipTalks, ...risingTalks];
            const isWorkshop = (t: SpeakerTalk) =>
              t.type === "Workshop" ||
              (Array.isArray(t.type) && t.type.includes("Workshop"));
            ...
            return (
              <div className="flex flex-col gap-4">
                <SL>{sectionLabel}</SL>
                {[
                  ...leadershipTalks.map((t) => ({
                    talk: t,
                    event: "leadership" as const,
                    light: false,
                  })),
                  ...risingTalks.map((t) => ({
                    talk: t,
                  ...
                  .sort((a, b) => {
                    const aW =
                      a.talk.type === "Workshop" ||
                      (Array.isArray(a.talk.type) &&
                        a.talk.type.includes("Workshop"));
                    const bW =
                      b.talk.type === "Workshop" ||
                      (Array.isArray(b.talk.type) &&
                        b.talk.type.includes("Workshop"));
