...
import { risingScheduleData } from "@/data/events/rising-leaders-schedule";
...
export default function RisingSchedule() {
  ...
  const currentSessions =
    activeDay === "day1" ? scheduleData.day1 : scheduleData.day2;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-0">
        ...
        <div className="mb-10 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-10">
            ...
            <h2 className="font-leadership text-3xl md:text-4xl text-[#0D0D0D] mb-2">
              Rising Leaders Forum 2026
            </h2>
            <p className="font-sans text-sm text-[#0D0D0D]/50 italic">
              ...
              <div className="grid grid-cols-2 gap-2 md:flex md:gap-3">
                {scheduleData.days.map((day) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] md:text-xs mt-0.5 block ${activeDay === day.id ? "text-[#E85520]" : "text-[#0D0D0D]/50"}`}
                    >
                      {day.date}
                    </span>
                  </button>
                    ...
                    <div className="flex items-center justify-between mb-4 lg:hidden">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {session.talkCategory && (
                          <span className="px-3 py-1 bg-[#0D0D0D]/10 text-[#0D0D0D]/60 text-[10px] font-sans font-semibold rounded-full uppercase tracking-wider">
                            {session.talkCategory}
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-[#0D0D0D]/40 shrink-0 ml-2">
                        {session.time}
                      </p>
                    </div>
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {session.speaker?.role && (
                          <p className="font-sans text-xs text-[#0D0D0D]/70 mt-0.5">
                            {session.speaker.role}
                          </p>
                        )}
                      </div>
                      ...
                      {session.panelists?.map((panelist, pIndex) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {panelist.role && (
                              <p className="font-sans text-xs text-[#0D0D0D]/60 mt-0.5 line-clamp-2">
                                {panelist.role}
                              </p>
                            )}
                          </div>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6 lg:mx-0 lg:px-0 lg:grid lg:grid-cols-5 lg:overflow-visible scrollbar-hide">
                      {session.workshops.map((workshop, wIndex) => (
                        ...
                        >
                          ...
                          <div
                            className={`flex ${workshop.speaker?.role ? "items-start" : "items-center"} gap-2 mb-2`}
                          >
                            ...
                            <div className="flex-1">
                              <p className="font-sans text-xs text-[#0D0D0D]/80 font-medium">
                                {workshop.speaker?.name || "TBA"}
                              </p>
                              {workshop.speaker?.role && (
                                <p className="font-sans text-[10px] text-[#0D0D0D]/60">
                                  {workshop.speaker.role}
                                </p>
                              )}
                            </div>
                          ...
                          {workshop.description && (
                            <p className="font-sans text-xs text-[#0D0D0D]/60 leading-snug">
                              {workshop.description}
                            </p>
                          )}
                        </div>
                      ...
                      {session.sessions.map((sess, sIndex) => (
                        ...
                        >
                              ...
                              {sess.panelists.map((panelist, pIndex) => (
                                <div key={pIndex} className="flex gap-2">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="font-sans text-[11px] text-[#0D0D0D]/80 font-medium truncate">
                                      {panelist.name || "TBA"}
                                    </p>
                                    {panelist.role && (
                          ...
                          {sess.type === "spark" && sess.speaker && (
                            <div
                              className={`flex ${sess.speaker?.role ? "items-start" : "items-center"} gap-2 mt-3`}
                            >
                              {sess.speaker?.image ? (
                                <img
                                  src={sess.speaker.image}
                                  alt={sess.speaker?.name}
                                  className="w-8 h-8 rounded-full shrink-0 border border-[#0D0D0D]/15 object-cover"
                                  crossOrigin="anonymous"
                                />
                              ) : (
                                <div className="w-8 h-8 rounded-full shrink-0 bg-[#0D0D0D]/5 border border-[#0D0D0D]/15 flex items-center justify-center">
          ...
          {prevDay ? (
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block font-sans text-[11px] text-[#0D0D0D]/40 uppercase tracking-wider">
                  Previous
                </span>
                <span className="block font-sans text-sm text-[#0D0D0D]/80 truncate group-hover:text-[#0D0D0D] transition-colors">
                  {prevDay.label}
                </span>
              </span>
            </button>
