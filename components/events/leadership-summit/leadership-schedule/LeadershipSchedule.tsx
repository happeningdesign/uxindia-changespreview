...
import { leadershipScheduleData } from "@/data/events/leadership-summit-schedule";
...
export default function LeadershipSchedule() {
  ...
  const prevDay =
    activeDayIndex > 0 ? scheduleData.days[activeDayIndex - 1] : null;
  const nextDay =
    activeDayIndex < scheduleData.days.length - 1
      ? scheduleData.days[activeDayIndex + 1]
      : null;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-0">
        ...
        <div className="mb-10 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-10">
            <p className="font-sans text-xs font-semibold uppercase tracking-widest text-[#E85520] mb-3">
              Schedule
            </p>
            <h2 className="font-leadership text-3xl md:text-4xl text-white mb-2">
              Leadership Summit 2026
            </h2>
            <p className="font-sans text-sm text-white/50 italic">
              Please note: Speaker line-up and session timings are subject to
              change.
            </p>
          </div>
        </div>
              ...
              <div className="grid grid-cols-3 gap-2 md:flex md:gap-3">
                {scheduleData.days.map((day) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] md:text-xs mt-0.5 block ${activeDay === day.id ? "text-[#E85520]" : "text-white/50"}`}
                    >
                      {day.date}
                    </span>
                  </button>
        ...
        <div className="space-y-6">
          {(currentSessions as any[]).map((session, index) => (
            <div
              key={index}
            ...
            >
              ...
              <div className="hidden lg:block lg:col-span-2">
                <div className="text-sm font-sans text-white/50 sticky top-24">
                  {session.time}
                  {session.endTime && session.type === "workshops"
                    ? ` – ${session.endTime}`
                    : ""}
                </div>
              </div>
              ...
              <div className="lg:col-span-10">
                    ...
                    <div className="flex items-center justify-between mb-4 lg:hidden">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {session.talkCategory && (
                          <span className="px-3 py-1 bg-white/10 text-white/60 text-[10px] font-sans font-semibold rounded-full uppercase tracking-wider">
                            {session.talkCategory}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="font-sans text-sm text-white/60 mb-1">
                          {session.description || "Announcing Soon"}
                        </p>
                        <p className="font-sans text-sm text-white/80 font-medium">
                          {session.speaker?.name || "TBA"}
                        </p>
                      ...
                      <div className="hidden lg:flex flex-ow items-end gap-2">
                        <span className="px-3 py-1 bg-[#E85520]/20 text-[#E85520] text-[10px] font-sans font-semibold rounded-full uppercase tracking-wider">
                          {session.tag}
                        </span>
                        {session.talkCategory && (
                ...
                {session.type === "closing" && (
                  <div className="relative overflow-hidden rounded-2xl border border-white/10 min-h-[180px] flex flex-col justify-end">
                    <img
                      src="/images/schedule/closing-ceremony.webp"
                    ...
                    <div className="relative z-10 p-6 flex items-end justify-between gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-leadership text-xl text-white">
                            {session.title}
                          </p>
                          <p className="font-sans text-xs text-white/70 mt-0.5">
                            Closing remarks and key takeaways from the
                            conference
                          </p>
                        </div>
                      </div>
                      <p className="font-sans text-xs text-white/60 lg:hidden shrink-0">
                        {session.time}
                      </p>
                    </div>
                  </div>
                ...
                {session.type === "continuation" && (
                  <div className="bg-white/5 border border-white/10 rounded-xl p-6 flex items-start gap-4">
                    <svg
                      className="w-6 h-6 text-[#E85520] shrink-0 mt-0.5"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                {session.type === "break" &&
                  (() => {
                    ...
                    const nextStart = nextSession
                      ? parseTimeToMinutes(nextSession.time)
                      : null;
                    const durationLabel =
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="font-sans text-xs text-white/90 mt-0.5">
                                  Join us for an evening of curated
                                  conversations and connections
                                </p>
                              </div>
                            </div>
                            <p className="font-sans text-xs text-white/80 lg:hidden shrink-0">
                              {session.time}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="w-10 h-10 bg-[#E85520]/20 rounded-lg flex items-center justify-center shrink-0">
                          {isLunch ? (
                            <img
                              ...
                              style={{
                                filter:
                                  "invert(44%) sepia(79%) saturate(1000%) hue-rotate(349deg) brightness(95%)",
                              }}
                            />
                          ) : isRegistration ? (
                            <svg
                  ...
                  <div className="bg-gradient-to-br from-white/8 to-white/[0.02] border border-white/10 rounded-2xl p-6 hover:border-[#E85520]/30 transition-all">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
                      {session.panelists?.map((panelist, pIndex) => (
                        <div
                          key={pIndex}
                          className={`flex ${panelist.role ? "items-start" : "items-center"} gap-3`}
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {panelist.role && (
                              <p className="font-sans text-xs text-white/70 mt-0.5 line-clamp-2">
                                {panelist.role}
                              </p>
                            )}
                          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {session.workshops.map((workshop, wIndex) => (
                      ...
                      >
                        ...
                        <div
                          className={`flex ${workshop.speaker?.role ? "items-start" : "items-center"} gap-3 mb-4`}
                        >
                        ...
                        <p className="font-sans text-xs md:text-sm text-white/60 leading-relaxed">
                          {workshop.description || "Announcing Soon"}
                        </p>
                      </div>
                    ))}
                  </div>
                    ...
                    {session.sessions.map((sess, sIndex) => (
                      ...
                      >
                        ...
                        {sess.panelists ? (
                            ...
                            {sess.panelists.map((panelist, pIndex) => (
                              <div key={pIndex} className="flex flex-col">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <p className="font-sans text-xs text-white font-medium leading-tight truncate flex-1">
                                    {panelist.name || "TBA"}
                                  </p>
                                </div>
                                {panelist.role && (
                                  <p className="font-sans text-[10px] text-white/60 ml-10 line-clamp-1">
                                    {panelist.role}
                                  </p>
                                )}
                              </div>
                          ...
                          <div
                            className={`flex ${sess.speaker?.role ? "items-start" : "items-center"} gap-3 mb-4`}
                          >
                            <div className="w-10 h-10 rounded-full shrink-0 border border-white/15 bg-white/10 flex items-center justify-center">
                              {sess.speaker?.image ? (
                                <img
                        ...
                        <p className="font-sans text-xs md:text-sm text-white/60 leading-relaxed">
                          {sess.description || "Announcing Soon"}
                        </p>
                      </div>
                    ))}
                  </div>
                          ...
                          {sess.panelists?.map((panelist, pIndex) => (
                            <div key={pIndex} className="flex flex-col">
                              <div className="flex items-center gap-3 mb-1">
                                ...
                                <p className="font-sans text-xs text-white/80 font-medium truncate flex-1">
                                  {panelist.name || "TBA"}
                                </p>
                              </div>
                              {panelist.role && (
                                <p className="font-sans text-[9px] text-white/60 ml-11 line-clamp-1">
                                  {panelist.role}
                                </p>
                              )}
                            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-gradient-to-br from-white/8 to-white/[0.02] border border-white/10 rounded-2xl p-6 hover:border-[#E85520]/30 transition-all h-full flex flex-col">
                      ...
                      <p className="font-sans text-xs md:text-sm text-white/60 leading-relaxed mb-4">
                        {session.rightSession.description || "Announcing Soon"}
                      </p>
                      <div className="flex-1">
                        ...
                        <div className="space-y-3">
                          {session.rightSession.panelists?.map(
                            (panelist, pIndex) => (
                              <div
                                key={pIndex}
                                className={`flex ${panelist.role ? "items-start" : "items-center"} gap-3`}
          ...
          {/* End of day */}
          {activeDay !== "day2" &&
            !currentSessions.some((s) => s.type === "dayend") && (
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex items-center justify-between gap-4">
          {prevDay ? (
            <button
              onClick={() => setActiveDay(prevDay.id)}
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block font-sans text-[11px] text-white/40 uppercase tracking-wider">
                  Previous
                </span>
                <span className="block font-sans text-sm text-white/80 truncate group-hover:text-white transition-colors">
                  {prevDay.label}
                </span>
              </span>
            </button>
          ...
          {nextDay ? (
            <button
              onClick={() => setActiveDay(nextDay.id)}
